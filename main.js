      ...
      getAnswers: (qids) => {
          ...
          GM.xmlHttpRequest({
              ...
              onload: (r) => {
                  ...
                  if (state.config.iqp) {
                    const img = body.querySelector('img[data-testid="attachments-viewer-image-preview"]');
                    if (img){
                      console.log(img, 'img yay')
                      let th = `<th>${e.textContent} ${rating} <a target="_blank" href="${img.src}">img</a></th>`
                      tr.innerHTML = th;
                    } else {
                      let th = `<th>${e.textContent} ${rating}</th>`
                      tr.innerHTML = th;
                    }
                  } else {
                    let th = `<th>${e.textContent} ${rating}</th>`
